...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        // Insert tester
        const testerResult = await client.query('INSERT INTO testers (name, role) VALUES ($1, $2) RETURNING id', [formData.testerInfo.name, formData.testerInfo.role]);
        const testerId = testerResult.rows[0].id;
        // Insert submission
        const submissionResult = await client.query('INSERT INTO submissions (tester_id, submission_date, overall_rating, final_suggestions) VALUES ($1, $2, $3, $4) RETURNING id', [
            testerId,
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            if (section) {
                const sectionResult = await client.query('INSERT INTO test_sections (submission_id, section_name, uiux_rating, comments) VALUES ($1, $2, $3, $4) RETURNING id', [
                    submissionId,
                    sectionNames[i],
                    ((_a = section.uiuxRating) === null || _a === void 0 ? void 0 : _a.rating) || 50,
                    section.comments || ''
                ]);
                ...
                if (section.features) {
                    for (const [featureName, featureData] of Object.entries(section.features)) {
                        await client.query('INSERT INTO feature_tests (section_id, feature_name, test_status) VALUES ($1, $2, $3)', [sectionId, featureName, featureData.status]);
                    }
                }
            }
        }
        ...
        if (formData.bugReports && formData.bugReports.length > 0) {
            for (const bug of formData.bugReports) {
                await client.query('INSERT INTO bug_reports (submission_id, priority, description, screenshot_url) VALUES ($1, $2, $3, $4)', [submissionId, bug.priority, bug.description, bug.screenshot || null]);
            }
        }
